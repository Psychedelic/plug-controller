...
export class Network {
  ...
  public getNftInfo = async ({ canisterId, identity, standard }) => {
    ...
    this.registeredNFTS = uniqueTokens([
      ...this.registeredNFTS,
      nft,
    ]) as RegisteredNFT[];
    return nft;
  ...
  public registerNFT = async ({ canisterId, standard, walletId, identity }) => {
    const nft = this.registeredNFTS.find(
      ({ canisterId: id }) => id === canisterId
    );
    if (!nft) {
      await this.getNftInfo({ canisterId, identity, standard });
    }
    this.registeredNFTS = this.registeredNFTS.map(n =>
      n.canisterId === canisterId
        ? { ...n, registeredBy: [...n?.registeredBy, walletId] }
        : n
    );
    await this.onChange?.();
  ...
  public registerToken = async ({
    ...
    const token = this.registeredTokens.find(
      ({ canisterId: id }) => id === canisterId
    );
    const defaultToken = this.defaultTokens.find(
      ({ canisterId: id }) => id === canisterId
    );
    if (defaultToken) {
    ...
    this.registeredTokens = this.registeredTokens.map(t =>
      t.canisterId === canisterId
        ? { ...t, logo, registeredBy: [...t?.registeredBy, walletId] }
        : t
    );
    await this.onChange?.();
  ...
  public removeToken = async ({
    ...
    const newTokens = this.registeredTokens.filter(
  ...
  constructor(
    {
      registeredTokens,
      onChange,
    }: { registeredTokens?: RegisteredToken[]; onChange?: () => void },
    fetch: any
  ) {
    super(
      {
