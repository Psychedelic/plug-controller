...
import { KeyringStorage } from '../../../interfaces/plug_keyring';
...
const createNetwork = (fetch, network) =>
  network.id === 'mainnet'
    ? new Mainnet(network, fetch)
    : new Network(network, fetch);
const createNetworks = (
  ...
  if (Object.values(networks || {})?.length) {
    return Object.values(networks!)?.reduce(
      (acum, net) => ({
        ...acum,
        [net.id]: createNetwork(fetch, { ...net, onChange }),
      }),
      {}
    );
  }
...
class NetworkModule {
  ...
  public addNetwork(networkParams: NetworkParams) {
    ...
    // }
    if (
      Object.values(this.networks).some(net => net.host === networkParams.host)
    ) {
      throw new Error(
        `A Network with host ${networkParams.host} already exists`
      );
    }
    const network = createNetwork(this.fetch, {
      ...networkParams,
  ...
  public removeNetwork(networkId: string) {
    ...
    if (!Object.keys(this.networks).includes(networkId))
      throw new Error(ERRORS.INVALID_NETWORK_ID);
