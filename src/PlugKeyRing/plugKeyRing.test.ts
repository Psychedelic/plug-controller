...
describe('Plug KeyRing', () => {
    ...
    describe('import', () => {
        ...
        it('should import the same wallet even with different passwords', async () => {
            const wallet = await keyRing.importMnemonic({ mnemonic: TEST_MNEMONIC, password: TEST_PASSWORD });
            const newWallet = await keyRing.importMnemonic({ mnemonic: TEST_MNEMONIC, password: 'newpassword1' });
    ...
    describe('storage', () => {
        it('should persist data encypted correctly after creating a new keyring', async () => {
            ...
            expect(CryptoJS.AES.decrypt(encryptedState, TEST_PASSWORD).toString(CryptoJS.enc.Utf8))
        ...
        it('should persist data encypted correctly after importing a keyring', async () => {
            await keyRing.importMnemonic({ mnemonic: TEST_MNEMONIC, password: TEST_PASSWORD });
            await keyRing.unlock(TEST_PASSWORD);
            const state = await keyRing.getState();
            const encryptedState = CryptoJS.AES.encrypt(JSON.stringify(state), TEST_PASSWORD);
            const stored = store.get();
