...
import {
  ...
  RegisteredToken,
  uniqueTokens,
} from '../PlugKeyRing/modules/NetworkModule/Network';
...
class PlugWallet {
  ...
  type: IDENTITY_TYPES;
  protected identity: GenericSignIdentity;
  ...
  get accountId(): string {
    return getAccountId(Principal.fromText(this.principal));
  }
  ...
  private async populateAndTrimNFTs(
    collections: NFTCollection[]
  ): Promise<WalletNFTCollection[]> {
    const icnsAdapter = new ICNSAdapter(this.agent);
    const collectionWithTokens = await getTokensFromCollections(
      this.network.registeredNFTS,
      this.principal,
      this.agent
    );
    ...
    ]) as WalletNFTCollection[];
    const simplifiedCollections = unique.map(
      (collection: NFTCollection): WalletNFTCollection => ({
        ...collection,
    ...
    const completeCollections = simplifiedCollections.filter(
      collection => collection.tokens.length > 0
    );
    return completeCollections;
  };

  private async nativeGetNFTs() {
    try {
  ...
  public async getTokenInfo({ canisterId, standard }) {
  ...
  public async getNFTInfo({ canisterId, standard }) {
    const nft = await this.network.getNftInfo({
      canisterId,
  ...
  public async registerNFT({
    canisterId,
    standard,
  }): Promise<RegisteredNFT[]> {
    const nfts = await this.network.registerNFT({
      canisterId,
      standard,
  ...
  public async registerToken(args: {
    canisterId: string;
    standard: string;
    logo?: string;
   ...
   */
  public async getBalances(): Promise<Array<TokenBalance>>{
  ...
  public async getTransactions(): Promise<GetTransactionsResponse> {
    ...
    transactionsGroup = transactionsGroup.map(tx => ({
      ...
      details: {
        ...tx.details,
        token:
          tx.details?.canisterId &&
          this.network.tokens.find(
            token => token.canisterId === tx.details?.canisterId
          ),
      },
    }));
  ...
  public async getICNSData(): Promise<{
    names: string[];
  ...
  public async getReverseResolvedName(): Promise<string | undefined> {
    const icnsAdapter = new ICNSAdapter(this.agent);
    return icnsAdapter.getICNSReverseResolvedName();
  };

  public async setReverseResolvedName({
    name,
  }: {
    name: string;
  }): Promise<string> {
  ...
  public async getContacts(): Promise<Array<Address>> {
    if (this.network.isCustom) return [];
    try {
  ...
  public async delegateIdentity(args: {
    to: Buffer;
    targets: string[];
  }): Promise<string> {
